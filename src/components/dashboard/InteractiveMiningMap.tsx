...
export function InteractiveMiningMap() {
  ...
  return (
    ...
    >
      ...
      {/* Marqueurs superposés sur la carte */}
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        {sites.map(site => {
          const position = projectToPixel(site.lat, site.lng);
          return (
            <div
              key={site.id}
              className="absolute transform -translate-x-1/2 -translate-y-1/2 pointer-events-auto cursor-pointer hover:scale-125 transition-transform duration-200"
              style={{
                left: `${position.x}%`,
                top: `${position.y}%`,
              }}
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg ${
                site.status === "alert" ? "bg-destructive" : "bg-primary"
              }`}>
